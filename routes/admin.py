...
def dashboard_finanzas():
    ...
    # Consulta CORREGIDA de empleados - JOIN con tabla empleado
    cursor.execute("""
        SELECT 
            u.id_usuario, 
            u.nombre, 
            u.ap_paterno,
            COALESCE(e.salario, 1800.00) as salario,
            COALESCE(e.banco, 'No especificado') as banco,
            COALESCE(e.numero_cuenta, 'No especificado') as numero_cuenta,
            COALESCE(e.puesto, 'No especificado') as puesto
        FROM usuario u
        LEFT JOIN empleado e ON u.id_usuario = e.id_usuario
        WHERE u.id_rol = 2 AND (e.estado IS NULL OR e.estado = 'activo')
    """)
    ...
    for row in cursor.fetchall():
        empleados.append({
            ...
            'nombre': row[1],
            'ap_paterno': row[2],
            'salario': float(row[3]) if row[3] else 1800.00,
            'banco': row[4],
            'numero_cuenta': row[5],
            'puesto': row[6]
        })

    # Consulta de residentes (sin cambios)
...
def pagar_empleado():
    ...
    try:
        ...
        nro_trans = request.form.get("nro_trans", "").strip()
        ...
        try:
            ...
            # Si no se proporciona monto, usar el salario del empleado
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT salario FROM empleado WHERE id_usuario = %s", (id_usuario,))
            empleado_salario = cursor.fetchone()
            monto = empleado_salario[0] if empleado_salario and empleado_salario[0] else 1800.00
            cursor.close()
            conn.close()
        ...
        numero_cuenta = empleado_info[0]
        banco = empleado_info[1]
        salario_base = empleado_info[2] or 0
        nombre_empleado = f"{empleado_info[3]} {empleado_info[4]}"
        puesto = empleado_info[5]

        # Procesar según el método de pago
        if metodo == "transferencia":
            if not numero_cuenta:
                flash(f"❌ El empleado {nombre_empleado} no tiene número de cuenta registrado", "danger")
                return redirect(url_for("admin.dashboard_finanzas"))
            
            if not banco:
                flash(f"⚠️ El empleado {nombre_empleado} no tiene banco registrado", "warning")
            
            # Si no se proporcionó número de transacción, generar uno automático
            if not nro_trans:
                nro_trans = f"TRF-{numero_cuenta[-4:]}-{datetime.now().strftime('%H%M')}"
            else:
                nro_trans = f"{banco or 'BANCO'}-{nro_trans}" if banco else nro_trans
            
            flash(f"💰 Transferencia a: {numero_cuenta} ({banco or 'Sin banco especificado'})", "info")

        elif metodo == "efectivo":
            if not nro_trans:
                nro_trans = f"EFECTIVO-{datetime.now().strftime('%Y%m%d%H%M')}"
            flash("💵 Pago en efectivo registrado", "info")

        elif metodo == "QR":
            if not nro_trans:
                nro_trans = f"QR-{datetime.now().strftime('%Y%m%d%H%M')}"
            flash("📱 Pago por QR registrado", "info")

        # Verificar coincidencia con salario base (solo advertencia)
        if salario_base > 0 and abs(monto - salario_base) > 1:  # Margen de error de 1 Bs
            flash(f"⚠️ El monto (Bs {monto:.2f}) difiere del salario base (Bs {salario_base:.2f})", "warning")
        ...
        flash(f"✅ Pago a {nombre_empleado} ({puesto}): Bs {monto:.2f} - {metodo.title()}", "success")
    ...
    try:
        ...
        if conn is None:
            ...
            return render_template("administrador/reporte.html",
                                 reportes_mensuales=[],
                                 deudas_info={'total_deudores': 0, 'deuda_total': 0, 'deuda_promedio': 0, 'total_deudas_pendientes': 0},
                                 residentes_deudores=[])
        ...
        # OBTENER LISTA DE RESIDENTES DEUDORES
        cursor.execute("""
            SELECT 
                u.id_usuario,
                u.nombre,
        ...
        """)
        residentes_deudores = cursor.fetchall()
        ...
        return render_template("administrador/reporte.html",
                             ...
                             residentes_deudores=residentes_deudores)
    except Exception as e:
...
def registrar_cobros():
    ...
    try:
        ...
        for concepto, monto, identificador in conceptos_lista:
            # Usar la estructura REAL de tu tabla deuda
            cursor.execute("""
                INSERT INTO deuda (
                    id_usuario, 
                    identificador, 
                    monto, 
                    estado
                    -- fecha_creacion NO EXISTE en tu tabla
                    -- invoice_url e invoice_id son opcionales
                )
        ...
        # Asegurar cierre de conexiones en caso de error
        if cursor:
            cursor.close()
        if conn:
            conn.close()
